...
class WeatherIconMapper {
  ...
  static Color glowFor(int conditionId) {
    if (conditionId == 800 || conditionId == 801) return kOrange.withValues(alpha: 0.3);
    if (conditionId >= 200 && conditionId < 600) return kTextPrimary.withValues(alpha: 0.3);
    if (conditionId >= 600 && conditionId < 700) return kTextSecond.withValues(alpha: 0.3); // snow: ice blue
    if (conditionId >= 700 && conditionId < 800) return kTextSecond.withValues(alpha: 0.3); // fog/mist: grey-blue
    return kTextSecond.withValues(alpha: 0.3); // clouds: steel blue
  }
}
